...
const Login = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400 ">
      ...
      />
      <div className="bg-slate-600 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-100 text-sm">
        <h2 className="text-3xl font-semibold text-white text-center mb-3">
          {state === "Sign Up" ? "Register Now" : "Login"}
        </h2>
       ...
       {state==='Sign Up'?( <p className="text-gray-400 text-center text-xs mt-4">Already have an account?{' '}  <span onClick={()=> setState('Login')} className="text-blue-500 cursor-pointer underline">login here</span></p>) : (<p className="text-gray-400 text-center text-xs mt-4">Don't have an account?{' '}  <span onClick={()=> setState('Sign Up')} className="text-blue-500 cursor-pointer underline">sign Up</span></p>)}
